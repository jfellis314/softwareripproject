...
public class Orders {
	...
	@FXML
	private TableColumn<OrdersDB, Integer> colOrderID;
	@FXML
	private TableColumn<OrdersDB, Integer> colPatient;
	@FXML
	...
	private void btnRefreshClicked(ActionEvent event) throws SQLException {
		...
		try(Connection conn = (Connection) sql.establishConn()){
			...
			ResultSet rs = orders.executeQuery("select order_id, patient, issuer, name, notes, order_name from appointments, modalities, order_status;");
			while(rs.next()) {
				data.add(new OrdersDB(rs.getInt(1), rs.getInt(2), rs.getString(3), rs.getString(4), rs.getString(5), rs.getString(6)));
			}
		...
		colReferral_md.setCellValueFactory(new PropertyValueFactory<>("issuer"));
