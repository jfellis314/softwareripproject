...
public class DiagnosticReports {
	...
	@FXML
	private TableColumn<DiagnosticReportsDB, Integer> colDiagReport;
	@FXML
	private TableColumn<DiagnosticReportsDB, Integer> colRadiologist;
	@FXML
	private TableColumn<DiagnosticReportsDB, Integer> colOrder;
	@FXML
	private TableColumn<DiagnosticReportsDB, Integer> colReport;
	@FXML
	...
	private void btnRefreshClicked(ActionEvent event) throws SQLException {
		...
		try(Connection conn = (Connection) sql.establishConn()){
			...
			while(rs.next()) {
				data.add(new DiagnosticReportsDB(rs.getInt(1), rs.getInt(2), rs.getInt(3), rs.getInt(4), rs.getString(5)));
			}
		...
		colRadiologist.setCellValueFactory(new PropertyValueFactory<>("order_id"));
		colOrder.setCellValueFactory(new PropertyValueFactory<>("patient"));
		colReport.setCellValueFactory(new PropertyValueFactory<>("radiologist"));
		colPatient.setCellValueFactory(new PropertyValueFactory<>("diagnostic"));
