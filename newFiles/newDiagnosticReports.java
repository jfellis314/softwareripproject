...
public class newDiagnosticReports {
	...
	public void btnSubmitClicked(ActionEvent event) throws IOException {
		...
		String radio = text2.getText();
		String order = text3.getText();
		String report = text4.getText();

		sql_conn sc = new sql_conn();
		try(Connection conn = (Connection) sc.establishConn())
		{
			Statement stmnt = conn.createStatement();
			stmnt.executeUpdate("INSERT INTO diagnostic_reports(diagnostic_report_id, order_id, radiologist, diagnostic) VALUES ("+DID+", "+order+", "+radio+", \""+report+"\")");
