...
public class newSystemUsers {
	...
	public void btnSubmitClicked(ActionEvent event) throws IOException {
		...
		String display = text3.getText();
		String email = text4.getText();
		String role = text5.getText();
		sql_conn sc = new sql_conn();
		String pw= text6.getText();
		...
		try(Connection conn = (Connection) sc.establishConn())
		{
			...
			stmnt.executeUpdate("INSERT INTO users(user_id, username, full_name, email, password, insurance) VALUES ("+UID+", \""+user+"\", \""+display+"\", \""+email+"\", \""+pw+ "\")");
			stmnt.executeUpdate("INSERT INTO users_roles(user_id, role_id) VALUES ("+UID+", "+role+")");
